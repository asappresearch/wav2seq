...
def get_aligned_bpe(bpe_ids: Union[str, List[int]], cids: Union[str, List[int]],
                    tokenizer: Tokenizer, chr_shift: int = 33):
    """
    args:
...
class W2V2FeatureReader(object):
    def __init__(self, ckpt_path, layer, max_chunk=1600000, fp16=False, pool_k=1, pool_s=1):
        ...
        elif hasattr(self.model, 'w2v_encoder') and hasattr(self.model.w2v_encoder, 'w2v_model'):
...
def get_reader(ckpt_path, layer, max_chunk, fp16, pool_k, pool_s):
    ...
    if ckpt_path == 'fbank':
        ...
        return W2V2FeatureReader(ckpt_path, layer, max_chunk, fp16=fp16, pool_k=pool_k, pool_s=pool_s)
...
def dump_hubert_features(
    ...
    layer=9, nshard=1, rank=0, max_chunk=1_600_000,
    fp16=False, pool_k=1, pool_s=1):
    reader = get_reader(ckpt_path, layer, max_chunk, fp16=fp16, pool_k=pool_k, pool_s=pool_s)
...
def dump_hubert_clusters(
    ...
    dump_cluster(rank, reader, apply_kmeans, manifest,
                 nshard, lab_dir, output_suffix)
...
def dump_hubert_features_pl(
    ...
    dump_cluster(rank, reader, apply_kmeans, manifest,
                 nshard, lab_dir, output_suffix, pl_converter=pl_converter)
...
def dump_aligned_pl(pl_folder='', bpe_suffix='chrd_bpe30000', bpe_path=''):
    ...
    splits = [filename.replace(suffix, '') for filename in os.listdir(pl_folder) if filename.endswith(suffix)]
    ...
    for split in splits:
        ...
        with open(output_file, 'w') as f:
            for cids, bpe_ids in tqdm(zip(clusters, bpes), total=len(bpes),
                                      desc=f"generating {output_file}"):
