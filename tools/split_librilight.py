...
def split_file(files, ext='flac', sample_rate=16000, min_duration=10., max_duration=30.):
    input_file, output_prefix = files
    if not os.path.exists(f'{output_prefix}.done'):
        data, _ = librosa.load(input_file, sample_rate)
        min_length, max_length = int(sample_rate * min_duration), int(sample_rate * max_duration)

        os.makedirs(os.path.dirname(output_prefix), exist_ok=True)

        for i, (s, e) in enumerate(get_start_ends(len(data), min_length, max_length)):
            soundfile.write(f'{output_prefix}_{i}.{ext}', data[s:e], samplerate=sample_rate)
        os.system(f'touch {output_prefix}.done')


def split_folder(input_dir, output_dir, njobs=6, ext='flac', sample_rate=16000, min_duration=10., max_duration=30.):
    ...
    for (dirpath, dirnames, filenames) in os.walk(input_dir):
        for filename in filenames:
            if filename.endswith('.flac'):
                ...
                input_filenames.append((filename, filename.replace(input_dir, output_dir).replace('.flac', '')))

    print(input_filenames[:10])

    func = partial(split_file, ext=ext, sample_rate=sample_rate,
