...
class Wav2Vec2AsrConfig(FairseqDataclass):
    ...
    # use Primer-EZ
    use_squared_relu: bool = field(
        default=False, metadata={"help": "use squared-relu activation in FFN"},
    )
    use_mdha: bool = field(
...
@register_model("wav2vec_ctc_v2", dataclass=Wav2Vec2CtcConfig)
class Wav2VecCtc(BaseFairseqModel):
    ...
    def build_model(cls, cfg: Wav2Vec2CtcConfig, task: FairseqTask):
        ...
        if cfg.pretrained_ckpt:
            ...
            # override the model archiecture
            with open_dict(model_cfg.w2v_args):
                for k in enc_overwrite:
                    if hasattr(cfg.w2v_args, k):
                        setattr(model_cfg.w2v_args, k, getattr(cfg.w2v_args, k))

            with open_dict(cfg):
                setattr(cfg, "w2v_args", model_cfg.w2v_args)
...
@register_model("wav2vec_seq2seq_v2", dataclass=Wav2Vec2Seq2SeqConfig)
class Wav2Vec2Seq2SeqModel(FairseqEncoderDecoderModel):
    def __init__(self, encoder, decoder, freeze_decoder=False):
    ...
    def build_model(cls, cfg: Wav2Vec2Seq2SeqConfig, task: FairseqTask):
        ...
        if cfg.pretrained_ckpt:
            ...
            with open_dict(cfg):
                for k in model_overwrite:
                    if hasattr(model_cfg, k):
                        setattr(cfg, k, getattr(model_cfg, k))
        ...
        decoder_embed_tokens = build_embedding(tgt_dict, model_cfg.decoder_embed_dim)

        encoder = cls.build_encoder(model_cfg)
        decoder = cls.build_decoder(model_cfg, tgt_dict, decoder_embed_tokens)

        encoder.w2v_model.mask_prob = cfg.mask_prob

        model = cls(encoder, decoder, freeze_decoder=cfg.freeze_decoder)
    ...
    def forward(self, **kwargs):
        ...
        if self.freeze_decoder:
            ...
            for p in self.decoder.layers.parameters():
                if p.requires_grad == ft:
                    # already set, skip it
                    break
                p.requires_grad = ft
    ...
    def extract_features(self, *args, **kwargs):
        return self.encoder.w2v_model.extract_features(*args, **kwargs)


@register_model("wav2vec_ctc_seq2seq", dataclass=Wav2Vec2Seq2SeqConfig)
class Wav2VecCtcSeq2Seq(Wav2Vec2Seq2SeqModel):
    def __init__(self, encoder, decoder, freeze_decoder=False):
        super().__init__(encoder, decoder, freeze_decoder)
...
class Wav2VecEncoder(FairseqEncoder):
    def __init__(self, cfg: Wav2Vec2AsrConfig, output_size=None):
        ...
        if cfg.drop_upsample:
            model.encoder.upsample = None
        ...
        self.output_dim = d if targ_d is None else targ_d
    ...
    def forward(self, source, padding_mask, **kwargs):
        # # hand empty input bug in dataloader
        # if source.shape[0] == 0:
        #     return {
        #         "encoder_out": source.new(*source.shape, self.cfg.decoder_embed_dim),  # T x B x C
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            ...
            if isinstance(res, dict):
                ...
                mask_indices = res.get("mask_indices", None)
            else:
                x, padding_mask = res
                mask_indices = None
    ...
    def extra_repr(self):
        try:
            return f"mask_prob={self.w2v_model.mask_prob}, mask_length={self.w2v_model.mask_length}"
        except:
            return ""
